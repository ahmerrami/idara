...
class OrdreVirementForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if instance:
            # En modification, inclure toujours le bénéficiaire déjà associé à l'OV,
            # même s'il n'entre plus dans les filtres courants.
            self.fields['beneficiaire'].queryset = Beneficiaire.objects.filter(
                Q(actif=True) | Q(pk=instance.beneficiaire_id)
            ).order_by(Lower('raison_sociale'))
            self.fields['beneficiaire'].initial = instance.beneficiaire_id
        else:
            # En création, limiter aux bénéficiaires ayant des factures en attente.
            self.fields['beneficiaire'].queryset = Beneficiaire.objects.filter(
                actif=True,
                factures_beneficiaire__statut='attente'
